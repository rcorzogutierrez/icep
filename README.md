...
- **Profesor**: define la rúbrica de una materia (categorías con peso, ej. "Tareas" 40%), crea tareas dentro de cada categoría, carga notas — individualmente o para toda la clase de una vez ("Revisar tarea") — y navega sus cursos y estudiantes de dos formas: por materia ("Mis estudiantes", roster a través de todas sus materias) o curso por curso ("Mis cursos"). Una vez que la rúbrica de un estudiante está completa, ve si quedó "Aprobado" o "Desaprobado" (C o mejor acredita la materia); si un curso está por terminar (o ya terminó) y todavía hay estudiantes sin nota final, aparece un aviso en Calificaciones y en "Mis cursos".
- **Estudiante**: ve sus materias, quién las dicta, su nota final y el desglose por categoría/tarea (calculado a partir de la rúbrica y las tareas calificadas), además de los comentarios del profesor — pueden ser varios a lo largo del tiempo (uno por cada instancia de evaluación, ej. "primer parcial", "segundo parcial"), cada uno con su fecha y, si el profesor lo eligió, la categoría de la rúbrica a la que se refiere. Puede elegir un apodo propio (`displayName`) en vez del nombre que trajo Google/email al registrarse.
...
- **`gradeHistory/{id}`** — registro inmutable de cada cambio real de nota (quién, cuándo, valor anterior y nuevo). Solo lo ve el staff; ni siquiera el admin puede editarlo o borrarlo una vez creado. Se consulta desde el botón de historial de cada fila en Calificaciones.
- **`gradeComments/{id}`** — comentarios del profesor a un estudiante en una materia: varios en el tiempo (no uno solo que se reemplaza), con fecha y, opcionalmente, la categoría de la rúbrica a la que se refieren. A diferencia de `gradeHistory`, el propio estudiante los puede leer — son para él.
