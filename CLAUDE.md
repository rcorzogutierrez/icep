...
**`onSnapshot`'s error callback must bind the error and `console.error` it**, not silently reset to empty state with a bare `() => {...}` — this used to be the app-wide pattern (an error, permission or otherwise, was indistinguishable from "no data" both to the user and to whoever's debugging afterward; this is literally how the missing-index bug above shipped unnoticed). The one deliberate exception: `UsersService` and `InvitationsService` subscribe for *any* logged-in user, not just staff, relying on `firestore.rules` to reject non-staff — for those two specifically, skip the log when `error.code === 'permission-denied'` (expected, every session, for every non-staff user) but still log anything else. Every other service gates its `effect()` on `isAdmin()/isTeacher()` before subscribing at all, so a `permission-denied` there is never expected — always log it. Same story for a bare `catch { ... }` around a `toast.error(...)` call: bind the error and `console.error` it before showing the (deliberately generic, translated) toast — otherwise a real bug and a network blip look identical in production with no way to tell them apart after the fact.
...
- `gradeCategories/{id}` (rubric, weighted categories — 100%-sum is validated client-side, not in rules) → `assignments/{id}` (a task in a category) → `grades/{subjectId}_{studentUid}` (deterministic id, `scores: assignmentId → points`, `comment: string | null` — the teacher's optional note, so a student can read their own grade with a direct `getDoc`).
- **A "single score" category's (`GradeCategory.hasMultipleTasks === false`) invisible backing assignment has `pointsPossible == category.weight`, not a fixed 100.** `GradeCategoriesService.create`/`update` set/sync this — if you touch either method, keep it in sync with the category's weight. This used to be hardcoded to 100 regardless of weight, which silently miscalculated every such category's grade (a teacher entering "9" for a 10%-weight category meant "9/10", but the system read it as "9/100") until caught and fixed; the UI showing that category's max points (`/{{category.weight}}` next to the score input) is the only thing that made the mismatch visible, so don't drop that label if you touch this UI.
...
- **Every mutating action needs a `toast.success(...)` on success, not just `toast.error(...)` on failure.** Six create/edit/delete handlers in `gradebook.ts` (rubric categories and assignments) shipped with error-only toasts — the action worked, but a user saving an assignment's due date got zero feedback either way and assumed it was broken. Check both branches when adding a save/delete handler, not just the `catch`.
- **A view where writing on every keystroke/blur is too eager (grading a whole class, editing a course's roster) stages edits in a local draft signal instead of writing immediately** — dirty state gets a visible highlight (row tint, badge) and an explicit save action; a "discard" escape hatch and a before/after confirmation summary are expected once more than a trivial value is at stake. See `CourseDetail`'s `pendingSummary`/leave-guard and `Gradebook`'s per-row `editingScores`/`confirmingRowUid` as the two reference implementations — copy one of these instead of wiring a bare `(change)` handler straight to Firestore.
