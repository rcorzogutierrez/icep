...
export class CourseSubjectsService {
  ...
  constructor() {
    effect((onCleanup) => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (error) => {
          console.error('[CourseSubjectsService] courseSubjects listener failed:', error);
          this._courseSubjects.set([]);
          this._loading.set(false);
        },
      );
  ...
  async unassign(id: string, batch?: WriteBatch): Promise<void> {
    const ref = doc(this.firestore, 'courseSubjects', id);
    if (batch) {
      batch.delete(ref);
      return;
    }
